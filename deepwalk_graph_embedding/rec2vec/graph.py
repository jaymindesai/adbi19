...
def records_to_graph():
    ...
    seq = [1]  # Used to generate a sequence of numbers for node ID's

    def _create_nodes(coll, type):
        """
            Create nodes and add them to the network represented by 'nodedict'
        """
        if type != 'movie':
            for name in coll:
                node = Node(seq[0], name, type)
                _add_node(name, node)
        else:
            for name, movie in movies.items():
                movie_node = Node(seq[0], name, type)
                _add_node(name, movie_node)

                for i in range(1, 6):
                    rating = name + '_' + str(i)
                    rating_node = Node(seq[0], rating, 'movie-rating')
                    _add_node(rating, rating_node)
                    _add_edge(rating_node, movie_node)

                for actor in movie.actors:
                    _add_edge(nodedict[actor], movie_node)

                for genre in movie.genres:
                    _add_edge(nodedict[genre], movie_node)

                if movie.director:
                    _add_edge(nodedict[movie.director], movie_node)

    def _add_node(key, node):
        """
    ...
    _create_nodes(ratings.keys(), 'user')
    _create_nodes(directors, 'director')
    _create_nodes(actors, 'actor')
    _create_nodes(genres, 'genre')
    _create_nodes(movies, 'movie')  # Movies must always be the last type to be added to the network
    ...
    for user, movie_ratings in ratings.items():
        for movie, rating in movie_ratings.items():
            key = movie + '_' + str(rating)
            _add_edge(nodedict[key], nodedict[user])
